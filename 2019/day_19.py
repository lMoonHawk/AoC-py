...
def get_program() -> list[int]:
    with open("2019/data/day_19.txt") as f:
...
def part1():
    program = get_program()
    return sum(run_intcode(program.copy(), [x, y]) for x, y in ((x, y) for x in range(50) for y in range(50)))
...
def part2():
    ...
    x, y = 0, 99
    while True:
        state = run_intcode(program.copy(), [x, y])
        if state:
            if run_intcode(program.copy(), [x + 99, y - 99]):
